...
trait Tables extends DBImpl {
  ...
  case class TmRoomConfigRow(gameType: Int, deviceType: Int, baseAmount: Int, minDiamondAmount: Int, maxDiamondAmount: Int, roomRanges: String = "", gmtCreate: java.sql.Timestamp, gmtUpdate: java.sql.Timestamp, topic: String)
  ...
  implicit def GetResultTmRoomConfigRow(implicit e0: GR[Int], e1: GR[String], e2: GR[java.sql.Timestamp]): GR[TmRoomConfigRow] = GR {
    prs => import prs._
      TmRoomConfigRow.tupled((<<[Int], <<[Int], <<[Int], <<[Int], <<[Int], <<[String], <<[java.sql.Timestamp], <<[java.sql.Timestamp], <<[String]))
  }
  ...
  class TmRoomConfig(_tableTag: Tag) extends profile.api.Table[TmRoomConfigRow](_tableTag, "tm_room_config") {
    def * = (gameType, deviceType, baseAmount, minDiamondAmount, maxDiamondAmount, roomRanges, gmtCreate, gmtUpdate, topic) <> (TmRoomConfigRow.tupled, TmRoomConfigRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(gameType), Rep.Some(deviceType), Rep.Some(baseAmount), Rep.Some(minDiamondAmount), Rep.Some(maxDiamondAmount), Rep.Some(roomRanges), Rep.Some(gmtCreate), Rep.Some(gmtUpdate), Rep.Some(topic)).shaped.<>({ r => import r._; _1.map(_ => TmRoomConfigRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
